...
export default function InvoicesPage() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10">
      ...
      {/* Header */}
      <div className="mb-6 flex items-start justify-between animate-in" style={{ animationDelay: "0ms" }}>
        <div>
      ...
      {/* Table */}
      <div
        className="overflow-x-auto rounded-xl border border-surface animate-in"
        style={{ animationDelay: "400ms" }}
      >
        <div className="min-w-[640px] overflow-hidden rounded-xl bg-surface-card">
          {filtered.length === 0 ? (
              ...
              <thead>
                <tr className="border-b border-surface bg-overlay-xs">
                  {["Invoice #", "Client", "Amount", "Status", "Due Date"].map((h) => (
                    <th key={h} className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-widest text-dim-app">
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {filtered.map((inv, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3.5 font-mono text-sm font-semibold text-primary-app transition-colors group-hover:text-violet-400">
                        {inv.invoice_number ?? "—"}
                      </td>
                      <td className="px-5 py-3.5 text-sm text-secondary-app group-hover:text-primary-app transition-colors">
                        {inv.client_id ? (clientMap[inv.client_id] ?? "—") : "—"}
                      </td>
                      <td className="px-5 py-3.5 text-sm font-bold text-bright tabular-nums">
                      ...
                      <td className="px-5 py-3.5 text-sm text-muted-app tabular-nums">
