...
export default function ProjectsPage() {
  ...
  async function load() {
    try {
      ...
      setProjects(p); setClients(c); setTasks(t);
    } catch (err: unknown) {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6">
      ...
      {/* Header */}
      <div className="flex items-center justify-between animate-in" style={{ animationDelay: "0ms" }}>
        <div>
      ...
      {error && (
        <div className="rounded-xl bg-rose-500/10 border border-rose-500/20 px-4 py-3 text-sm text-rose-400 animate-in">
          {error}
        </div>
      )}
      ...
      {projects.length === 0 ? (
        ...
        <div
          className="overflow-x-auto rounded-2xl border border-surface animate-in"
          style={{ animationDelay: "160ms" }}
        >
          <div className="min-w-[620px] rounded-2xl bg-surface-card overflow-hidden">
            {/* Column headers */}
            <div className="grid grid-cols-[1fr_140px_130px_100px_36px] gap-0 border-b border-surface bg-overlay-xs px-6 py-2.5">
              {["Project", "Status", "Deadline", "Progress", ""].map((h) => (
                <p key={h} className="text-[10px] font-semibold uppercase tracking-widest text-dim-app">{h}</p>
              ))}
            </div>
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      className="grid grid-cols-[1fr_140px_130px_100px_36px] items-center gap-0 px-6 py-4 hover:bg-overlay-sm transition-[background-color] duration-150 group"
                    >
                      {/* Name + client */}
